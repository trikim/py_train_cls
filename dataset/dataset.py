...
class CUB_200_2011_Train(Dataset):
    ...
    def __init__(self, path, path_txt, transform=None, target_transform=None):
        ...
        self.class_cnt = np.zeros(7)
        for folder in os.listdir(self.root):
            class_id = cls_names.index(folder)
            class_num = len(glob.glob(os.path.join(self.root, folder) + '/*.jpg'))
            cnt = 0
            for imgname in os.listdir(os.path.join(self.root, folder)):
                self.num_train = self.num_train + 1
                self.images_path[self.num_train] = folder + '/' + imgname
                self.class_ids[self.num_train] = class_id
                self.train_id.append(self.num_train)
                self.class_cnt[class_id] += 1
                cnt += 1
                if cnt >= int(class_num * 0.95):
                    break
        print('[train num count]:{}\t{}\t{}\t{}\t{}\t{}\t{}'.format(self.class_cnt[0],self.class_cnt[1],self.class_cnt[2],self.class_cnt[3],self.class_cnt[4],self.class_cnt[5],self.class_cnt[6]))
    ...
    def __getitem__(self, index):
        ...
        if image is None:
            print('Train Image Read Error: {}'.format(os.path.join(self.root, path)))
        #name = path.rsplit('/')[-1]
...
class CUB_200_2011_Test(Dataset):
    ...
    def __init__(self, path, path_txt, transform=None, target_transform=None):
        ...
        for folder in os.listdir(self.root):
            ...
            for imgname in os.listdir(os.path.join(self.root, folder)):
                cnt += 1
                if cnt < (class_num * 0.95):
                    continue
                self.num_test = self.num_test + 1
                self.images_path[self.num_test] = folder + '/' + imgname
                self.class_ids[self.num_test] = class_id
                self.test_id.append(self.num_test)
                self.class_cnt[class_id] += 1
        print('[test num count]:{}\t{}\t{}\t{}\t{}\t{}\t{}'.format(self.class_cnt[0],self.class_cnt[1],self.class_cnt[2],self.class_cnt[3],self.class_cnt[4],self.class_cnt[5],self.class_cnt[6]))
