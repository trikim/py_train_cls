...
try:
    ...
    DATA_BACKEND_CHOICES.append('pytorch')
except ImportError:
...
def get_dali_train_loader(dali_cpu=False):
    def gdtl(data_path, batch_size, num_classes, one_hot, workers=5, _worker_init_fn=None):
        if torch.distributed.is_initialized():
            local_rank = torch.distributed.get_rank()
            world_size = torch.distributed.get_world_size()
        else:
...
def get_dali_val_loader():
    def gdvl(data_path, batch_size, num_classes, one_hot, workers=5, _worker_init_fn=None):
        if torch.distributed.is_initialized():
            local_rank = torch.distributed.get_rank()
            world_size = torch.distributed.get_world_size()
        else:
...
def get_pytorch_train_loader(data_path, batch_size, num_classes, one_hot, workers=5, _worker_init_fn=None):
    traindir = os.path.join(data_path, 'train')
    ...
    return PrefetchedWrapper(train_loader, num_classes, one_hot), len(train_loader)

def get_pytorch_val_loader(data_path, batch_size, num_classes, one_hot, workers=5, _worker_init_fn=None):
    valdir = os.path.join(data_path, 'val')
    ...
    return PrefetchedWrapper(val_loader, num_classes, one_hot), len(val_loader)
